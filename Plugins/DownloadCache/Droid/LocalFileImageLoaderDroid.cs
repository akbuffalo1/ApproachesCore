...
{
    public class LocalFileImageLoaderDroid : ILocalFileImageLoader<Bitmap>
    {
        private const string TAG = "AD.Plugins.DownloadCache.Droid.LocalFileImageLoaderDroid";
        private const string ResourcePrefix = "res:";

        private readonly IDictionary<CacheKey, WeakReference<Bitmap>> _memCache =
            new Dictionary<CacheKey, WeakReference<Bitmap>>();

        public async Task<Image<Bitmap>> Load(string localPath, bool shouldCache, int maxWidth, int maxHeight)
        {
            Bitmap bitmap;
            ...
            return (Image<Bitmap>)new ImageDroid(bitmap);
        }

        private IAndroidGlobals _androidGlobals;

        protected IAndroidGlobals AndroidGlobals => _androidGlobals ?? (_androidGlobals = Resolver.Resolve<IAndroidGlobals>());
        ...
        private async Task<Bitmap> LoadResourceBitmapAsync(string resourcePath)
        {
            ...
            if (id == 0)
            {
                Resolver.Resolve<ILogger>().Error(TAG, string.Format("Value '{0}' was not a known drawable name", resourcePath));
                return null;
            }
