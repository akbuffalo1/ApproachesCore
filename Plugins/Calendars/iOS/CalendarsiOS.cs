...
namespace AD.Plugins.Calendars.iOS
{
    ...
    /// </summary>
    public class CalendarsiOS : ICalendars
    {
#region Constants
...
#region ICalendars Implementation
        ...
        public async Task<bool> RequestPermission()
        {
            ...
            {
                var status = EventKit.EKEventStore.GetAuthorizationStatus(EventKit.EKEntityType.Event);
                if (status == EventKit.EKAuthorizationStatus.Denied || status == EventKit.EKAuthorizationStatus.Restricted)
                    return false;
                if (_eventStore == null)
                {
                ...
                if (status == EventKit.EKAuthorizationStatus.NotDetermined)
                {
                    var result = await _eventStore?.RequestAccessAsync(EventKit.EKEntityType.Event);
        ...
        {
            await RequestPermission().ConfigureAwait(false);
        ...
        /// <exception cref="System.UnauthorizedAccessException">Calendar access denied</exception>
        /// <exception cref="Plugin.Calendars.Abstractions.PlatformException">Unexpected platform-specific error</exception>
        public async Task<Calendar> GetCalendar(string externalId)
        {
        ...
        /// <exception cref="System.UnauthorizedAccessException">Calendar access denied</exception>
        /// <exception cref="Plugin.Calendars.Abstractions.PlatformException">Unexpected platform-specific error</exception>
        public async Task<CalendarEvent> GetEvent(string externalId)
        {
            if (string.IsNullOrWhiteSpace(externalId))
            {
                return null;
