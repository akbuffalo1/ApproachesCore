...
namespace AD.TypeExtensions
{
	...
	public static class TypeExtensions
	{
		public static IEnumerable<Type> ExceptionSafeGetTypes(this Assembly assembly)
		{
			...
			{
				Debug.WriteLine(string.Format("ReflectionTypeLoadException masked during loading of {0} - error {1}",
		...
		public static IEnumerable<ServiceTypeAndImplementationTypePair> AsInterfaces(this IEnumerable<Type> types)
		{
			...
			return types.Select((t) => {
				Debug.WriteLine(t);
				Debug.WriteLine(t.GetInterfaces().ToList());
				return new ServiceTypeAndImplementationTypePair(t.GetInterfaces().ToList(), t);
			});
		}
		...
		public static void RegisterAsSingleton(this IEnumerable<ServiceTypeAndImplementationTypePair> pairs)
		{
			foreach (var pair in pairs)
			{
				...
				//var instance = AD.Resolver.Construct(pair.ImplementationType);
				foreach (var serviceType in pair.ServiceTypes)
				{
					//Mvx.RegisterSingleton(serviceType, instance);
					AD.Resolver.Resolve<IDependencyContainer> ().RegisterAsSingleton(serviceType, pair.ImplementationType);
				}
			}
		}

		public static void RegisterAsMultiInstance(this IEnumerable<ServiceTypeAndImplementationTypePair> pairs)
		{
			foreach (var pair in pairs)
			{
				foreach (var serviceType in pair.ServiceTypes)
				{
					...
					AD.Resolver.Resolve<IDependencyContainer> ().RegisterAsMultiInstance(serviceType, pair.ImplementationType);
				}
			}
		}
		...
		public static void RegisterAsDynamic(this IEnumerable<ServiceTypeAndImplementationTypePair> pairs)
		{
			foreach (var pair in pairs)
			{
				...
				{
					//Mvx.RegisterType(serviceType, pair.ImplementationType);
					AD.Resolver.Resolve<IDependencyContainer> ()
				}
			}
		}*/
